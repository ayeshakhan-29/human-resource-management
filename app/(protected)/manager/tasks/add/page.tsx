...
export default function ManagerAddTaskPage() {
  ...
  // Set manager ID when user is loaded
  useEffect(() => {
    if (user?.id && !form.managerId) {
      setForm(prev => ({ ...prev, managerId: user.id }));
    }
  }, [user?.id, form.managerId]);
    ...
    <>
      ...
      <div className="flex flex-1 flex-col gap-6 p-6">
        <div className="flex flex-col gap-2">
          ...
          <p className="text-gray-600">Create a new task for your team members in your managed projects</p>
        </div>
        ...
        <form onSubmit={onSubmit} className="space-y-6">
          <Card>
            ...
            <CardContent className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  {projects.length === 0 && (
                    <div className="text-sm text-amber-600">
                      No projects found. You need to be assigned as a manager to projects to create tasks.
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="p-4 bg-gray-50 rounded-lg">
                <Label className="text-sm font-medium text-gray-700">Task Manager</Label>
                <p className="text-sm text-gray-600 mt-1">
                  {user?.name || user?.fullName} (You) - This task will be managed by you
                </p>
              </div>
            </CardContent>
          </Card>
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-end">
            ...
            <Button type="submit" className="w-full sm:w-auto" disabled={loading || projects.length === 0}>
